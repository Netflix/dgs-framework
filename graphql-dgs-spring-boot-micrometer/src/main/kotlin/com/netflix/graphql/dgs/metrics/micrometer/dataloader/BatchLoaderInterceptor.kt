    ...
    override fun invoke(proxy: Any, method: Method, args: Array<out Any>): CompletionStage<List<*>> {
        if (method.name == "load") {
            ...
            return try {
                ...
                val future = method.invoke(batchLoader, *(args)) as CompletionStage<List<*>>
                future.whenComplete { result, _ ->
                    logger.debug("Finished timer[{}] for {}.", ID, javaClass.simpleName)
                    timerSampler.stop(
                        Timer.builder(ID)
                            .tags(
                                listOf(
                ...
                @Suppress("UNCHECKED_CAST")
                method.invoke(batchLoader, *(args)) as CompletionStage<List<*>>
            }
        }
