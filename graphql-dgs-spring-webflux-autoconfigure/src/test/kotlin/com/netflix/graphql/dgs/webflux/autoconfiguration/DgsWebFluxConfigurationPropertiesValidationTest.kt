...
class DgsWebFluxConfigurationPropertiesValidationTest {
    ...
    @Test
    fun graphiqlControllerInvalidCustomSubpathEndsWithSlash() {
        context
            .withPropertyValues("dgs.graphql.graphiql.subpath: /fooql/")
            .run { ctx ->
                assertThat(ctx).hasFailed().failure.getRootCause().hasMessageContaining("dgs.graphql.graphiql.subpath must start with '/' and not end with '/'")
            }
    }

    @Test
    fun graphiqlControllerInvalidCustomSubpathDoesNotStartWithSlash() {
        context
            .withPropertyValues("dgs.graphql.graphiql.subpath: fooql")
            .run { ctx ->
                assertThat(ctx).hasFailed().failure.getRootCause().hasMessageContaining("dgs.graphql.graphiql.subpath must start with '/' and not end with '/'")
    ...
    @Test
    fun graphiqlControllerValidCustomSubpath() {
        context
            .withPropertyValues("dgs.graphql.graphiql.subpath: /fooql")
            .run { ctx ->
                assertThat(ctx).hasNotFailed()
