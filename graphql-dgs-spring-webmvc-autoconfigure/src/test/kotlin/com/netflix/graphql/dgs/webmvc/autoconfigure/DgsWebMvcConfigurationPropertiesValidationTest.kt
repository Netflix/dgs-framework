    ...
    fun graphqlControllerInvalidCustomPathEndsWithSlash() {
        context
            ...
            .run { ctx ->
                assertThat(
                    ctx,
                ).hasFailed().failure.rootCause().hasMessageContaining("dgs.graphql.path must start with '/' and not end with '/'")
            }
    }
    ...
    fun graphiqlControllerInvalidCustomPathEndsWithSlash() {
        context
            ...
            .run { ctx ->
                assertThat(
                    ctx,
                ).hasFailed().failure.rootCause().hasMessageContaining("dgs.graphql.graphiql.path must start with '/' and not end with '/'")
            }
    }
    ...
    fun schemaJsonControllerInvalidCustomPathEndsWithSlash() {
        context
            ...
            .run { ctx ->
                assertThat(
                    ctx,
                ).hasFailed().failure.rootCause().hasMessageContaining(
                    "dgs.graphql.schema-json.path must start with '/' and not end with '/'",
                )
            }
    }
